public final class Task11 {
    ...
    public static void main(String[] args) {
        System.out.println(new Task11().run());
    }
    ...
    private static int product(int x, int y, int dx, int dy) {
        ...
        if (!isInBounds(x + (Task11.N - 1) * dx, y + (Task11.N - 1) * dy))
            return -1;

        int prod = 1;
        for (int i = 0; i < Task11.N; i++, x += dx, y += dy)
            prod *= DATA[y][x];
        return prod;
    }

    private static boolean isInBounds(int x, int y) {
        return 0 <= y && y < DATA.length && 0 <= x && x < DATA[y].length;
    }
